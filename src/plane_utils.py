...
from src.types import COMPTYPE, Point
...
def calculate_angle(p1: Point, vertex: Point, p3: Point) -> float | None:
    ...
    """
    if (
        double_compare(p1[0], vertex[0]) == COMPTYPE.EQ
        and double_compare(p1[1], vertex[1]) == COMPTYPE.EQ
    ) or (
        double_compare(p3[0], vertex[0]) == COMPTYPE.EQ
        and double_compare(p3[1], vertex[1]) == COMPTYPE.EQ
    ):
        return -1.0
